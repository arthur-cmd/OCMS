...
class _DetailscoState extends State<Detailsco> {
  Future accept() async {
    const url =
        'http://192.168.43.132:8080/ocms/head_of_unit_staff/bursar_accept.php';
    ...
    try {
      if (response.statusCode == 200) {
        Navigator.pushReplacement(
              context, MaterialPageRoute(builder: (_) => Bursar()));
  ...
  Widget _buildPopupDialog(BuildContext context) {
    ...
    Future decline() async {
      var from = 'Bursar';
      const url =
          'http://192.168.43.132:8080/ocms/head_of_unit_staff/bursar_staff_decline.php';
      var request = http.MultipartRequest('POST', Uri.parse(url));
      request.fields['names'] = widget.names.toString();
      request.fields['pay_roll'] = widget.pay_roll.toString();
